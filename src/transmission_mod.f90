...
#include <petsc/finclude/petsc.h>
    ...
    if ((solver_mode .eq. 1).or.(solver_mode .eq. 2)) then
    ...
    else if ((solver_mode .eq. 3).or.(solver_mode .eq. 4)) then
...
!~       write(pstr_out,fmt='(A,2e24.12)') "trans ",trans ; call petsc_print_master()
!
      call diag_mat(p_tt, PETSC_NULL_MAT, t_i(iik), PETSC_NULL_MAT, nmu_r)
