...
!~ AFAIK at the moment this is the only workaround
  ...
  type(Mat_pointer_array), allocatable :: p_h00_diag(:,:,:), p_s00_diag(:,:,:)
...
! for current density one maybe have to use the full matrices as we have to contraction with S^-1 -> S^-1*D*S^-1
  Mat, allocatable ::     p_dmatxy(:, :, :), p_dmatxy_l(:, :), p_dmatxy_r(:, :)
...
! possible to restrict to nonzero structure of Y or not
  subroutine petsc_aXpY(Y, X, a, laddnew, lt)
#include <petsc/finclude/petscmat.h>
    use globals, only: inode
    use error_handler
    implicit none
    ...
    PetscInt :: ncols, rows(1)
    PetscInt, allocatable :: cols(:)
    PetscBool :: laddnew_save, loffproc_save
    PetscErrorCode :: p_ierr
...
!~       end if
    ...
    call MatSetOption(Y, MAT_IGNORE_OFF_PROC_ENTRIES, PETSC_FALSE, ierr)
...
!~     call MatDestroy(p_preG, ierr)
    ...
    if (l_k_on_demand) then
      ...
      do ik = 1, nk_c
        call MatDestroy(p_h00k_cc(ik), ierr)
        call MatDestroy(p_s00k_cc(ik), ierr)

        call MatDestroy(p_h00k_l(ik), ierr)
        call MatDestroy(p_s00k_l(ik), ierr)
        call MatDestroy(p_h10k_l(ik), ierr)
        call MatDestroy(p_s10k_l(ik), ierr)
        call MatDestroy(p_h01k_l(ik), ierr)
        call MatDestroy(p_s01k_l(ik), ierr)

        call MatDestroy(p_h00k_r(ik), ierr)
        call MatDestroy(p_s00k_r(ik), ierr)
        call MatDestroy(p_h10k_r(ik), ierr)
        call MatDestroy(p_s10k_r(ik), ierr)
        call MatDestroy(p_h01k_r(ik), ierr)
        call MatDestroy(p_s01k_r(ik), ierr)
      end do
...
!~     end if
  ...
  subroutine dump_nonzero_structure(p_A, filename, comm, ionode)
