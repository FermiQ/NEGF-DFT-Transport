    ...
    do ik = nk_c, 1, -1
      ...
      if (((mul .ne. mur) .and. (vb .ne. 0d0)).and.(.not.l_no_noneq)) then
        ...
        if (ldouble_contour) then
          ...
          call MatAXPY(p_d_dc_k(1), p_minus1, p_d_tmp1(2), SAME_NONZERO_PATTERN, ierr)
        end if
...
! semi circle -------------------------------------
      call init_eq_int(mur, mur, 2) !1=left, 2=right, 3=average left and right fermi functions
...
! along E axis and fermi poles --------------------------
        ...
        if (ldouble_contour) then
         ...
         &  eps_int_contour, err_eq_int, subfile)
          ...
          call fourier_back_add(p_d_dc_k, p_dmatxy(:, :, 0), kp_l(1:2, ik:ik), wkp_l(ik:ik), 1, &
          dlat_l, ncell_c(1), ncell_c(2), 2, 0)
...
!~       call PetscViewerDestroy(viewer,ierr)
    ...
    if (ldouble_contour) then
      ...
      call MatDestroy(p_d_tmp1(2), ierr)
